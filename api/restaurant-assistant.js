...
function restaurantInput(history, message) {
  const cleanHistory = Array.isArray(history)
    ? history
        ...
        .map((e) => `${e.role === "assistant" ? "Restaurant assistant" : "Customer"}: ${e.content.slice(0, maxMessageLength)}`)
        .join("\n")
    : "";
...
export default async function handler(req, res) {
  ...
  if (!message || message.length > maxMessageLength) {
    return res.status(400).json({ error: "Please send a short restaurant question." });
  }

  if (!isRestaurantQuestion(message)) {
    return res.json({ reply: scopeReply, source: "scope" });
  }

  if (!process.env.GROQ_API_KEY) {
    return res.status(503).json({ error: "Set GROQ_API_KEY on the server before using the restaurant assistant." });
  }
  ...
  try {
    ...
    const response = await groq.responses.create({
      ...
      input: restaurantInput(req.body?.history, message),
    });
    const reply =
    ...
    if (!reply) {
      return res.status(502).json({ error: "I couldn't prepare a restaurant answer right now. Please call or message +92 328 8681123 for immediate help." });
    }
