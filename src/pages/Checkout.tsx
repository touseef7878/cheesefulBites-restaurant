...
export default function Checkout() {
  ...
  const [streetAddress, setStreetAddress] = useState("Lane 21, Phase-II Wah Model Town");
  const [city, setCity] = useState("Wah");
  const [postalCode, setPostalCode] = useState("47010");
  ...
  const placeOrder = () => {
    ...
    const message = buildWhatsAppOrderMessage({
      ...
      payment: payment === "cash" ? "Cash on Delivery" : "Digital Payment", instructions,
    });
      ...
      <section className="checkout-page content-wrap">
        ...
        <div className="checkout-layout">
          <div className="checkout-main">
            <motion.article className="delivery-card" {...reveal}><div className="delivery-card__heading"><h2><Truck />Delivery Details</h2><Link href="/contact">Edit</Link></div><div className="address-surface"><span><MapPin /></span><div><b>Cheeseful Bites Delivery</b><p>QP8J+V6R, Lane 21, Phase-II Wah Model Town, Wah, 47010, Pakistan</p></div></div><div className="delivery-inputs"><label>Street address<input value={streetAddress} onChange={(event) => setStreetAddress(event.target.value)} /></label><label>City<input value={city} onChange={(event) => setCity(event.target.value)} /></label><label>Postal Code<input value={postalCode} onChange={(event) => setPostalCode(event.target.value)} /></label><label className="full">Delivery instructions<textarea value={instructions} onChange={(event) => setInstructions(event.target.value)} placeholder="Leave notes for your order here..." /></label></div></motion.article>
            ...
            <motion.article className="payment-card" {...reveal}><h2><CreditCard />Payment Method</h2><button className={payment === "cash" ? "is-selected" : ""} onClick={() => setPayment("cash")}><span>●</span>Cash on Delivery <CreditCard /></button><button className={payment === "digital" ? "is-selected" : ""} onClick={() => setPayment("digital")}><span>○</span>Digital Payment <CreditCard /></button></motion.article>
          </div>
